...
class crew:
    ...
    def get_sub_domain(self,page_num=0):
        ...
        url = 'https://www.baidu.com/s?wd=inurl:%s&pn=%d&oq=inurl:%s'%(self.url,page_num,self.url)
        ...
        for one_url in one_page_urls:
            ...
            for i in one_url.strings:   # 提取标签中的内容
                try:
                    ...
                    pass
            # 子域名和存活性检测
            if sub_domain not in all_sub_domain and self.is_alive('http://' + sub_domain):
                all_sub_domain.append(sub_domain)
        if 'class="n">下一页' in content:
            new_page_num = page_num + 10
            url = 'https://www.baidu.com/s?wd=inurl:%s&pn=%s&oq=inurl:%s'%(self.url,new_page_num,self.url)
